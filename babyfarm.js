...
counters.forEach(counter => {
  counter.innerText = '0';
  const updateCounter = () => {
    const target = +counter.getAttribute('data-target'); // Convertit en nombre
    const current = +counter.innerText;
    const increment = target / 100; // Ajuste la vitesse

    if (current < target) {
      counter.innerText = `${Math.ceil(current + increment)}`;
      setTimeout(updateCounter, 70); // Appelle la fonction toutes les 20ms
    } else {
      counter.innerText = target; // S'assure d'atteindre la valeur cible
    }
  };
